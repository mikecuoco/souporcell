...
nclust <- clusters %>%
    ...
    ncol()
barcodes = nrow(clusters)
cells_loaded = 40000
expect_doublets = round((4.597701e-06 * cells_loaded) * barcodes)

glue("From loading {cells_loaded}, we expect {expect_doublets} doublets") %>% print()
glue("Souporcell returns {barcodes} total barcodes in {nclust} clusters") %>% print()
